    ...
    public class SubscriptionModule : BaseCommandModule
    {
        ...
        public async Task AllSubs(CommandContext ctx)
        {
            ...
            using (SQLDataBase context = new SQLDataBase(SteamUpdateBot.ConnectionString))
            {
                ...
                var allguilds = context.AllGuilds;

                foreach (var guild in allguilds)
                {
                    if (ctx.Guild != null && (ulong)guild.GuildID == ctx.Guild.Id)
                    {
                        foreach (var app in guild.SubscribedApps)
                        {
                            var channel = ctx.Guild.GetChannel((ulong)guild.ChannelID);
                    ...
                    else if ((ulong)guild.ChannelID == ctx.User.Id && ctx.Guild == null)
                    {
                        foreach (var app in guild.SubscribedApps)
                        {
                            stringBuilder.AppendLine($"{await SteamUpdateBot.SteamClient.GetAppName((uint)app.AppID)} ({app.AppID})");
